...
export interface ConstraintSolver {
  ...
  feasibleValues(partial: Map<number, number>, parameterIndex: number): number[];
  completeRow(partial: Map<number, number>): number[] | null;
...
export function createConstraintSolver(model: CanonicalModel): ConstraintSolver {
  ...
  const feasibleValues = (partial: Map<number, number>, parameterIndex: number): number[] => {
    const assignment = toConstraintAssignment(partial);
    if (!assignment) {
  ...
  const completeRow = (partial: Map<number, number>): number[] | null => {
    if (!canCompleteInternal(partial)) {
      return null;
    }
    ...
    const walk = (): number[] | null => {
      ...
      for (const valueIndex of feasibleValues(current, parameterIndex)) {
  ...
  return {
    canComplete(partial) {
      return canCompleteInternal(partial);
    },
    feasibleValues,
    completeRow,
    stats() {
